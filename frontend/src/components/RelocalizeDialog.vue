...
<template>
  <Teleport to="body">
    <Transition name="app-dialog">
      <div v-if="open" class="app-dialog-overlay" @mousedown="onOverlayMouseDown">
        <div class="app-dialog-card reloc-card" role="dialog" aria-modal="true">
          ...
          <div class="app-dialog-actions">
            <button v-if="step === 'result' && !running" type="button" @click="reloc.openDialog()">重试 / 换方式</button>
            <button type="button" :disabled="running" @click="reloc.close()">关闭</button>
            <button
              v-if="step === 'result' && r && !running && !applied"
              type="button"
              class="primary"
